...
// 已移除的示範案件：同步清除既有瀏覽器暫存，避免舊資料重新出現。
const deletedCaseIds=['EDU-2024-006','EDU-2024-005'];
const filteredCases=cases.filter(c=>!deletedCaseIds.includes(c.id));
if(filteredCases.length!==cases.length){cases=filteredCases;localStorage.setItem('review-cases',JSON.stringify(cases));renderCases();}
